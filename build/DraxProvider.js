...
const DraxProvider = ({ debug = false, style = styles.provider, children, }) => {
    ...
    const throttledHandleGestureEvent = (0, react_1.useMemo)(() => (0, lodash_throttle_1.default)((id, event) => {
        // Pass the event up to the Drax context.
        handleGestureEvent(id, event);
    }, 10), [handleGestureEvent]);
    const longPress = (0, react_1.useCallback)(({ id, longPressDelay, draggable }) => react_native_gesture_handler_1.Gesture
        .Pan()
        .onBegin((ev) => {
        ...
        .enabled(draggable)
        .maxPointers(1), [handleGestureStateChange, handleGestureEvent]);
    const contextValue = {
        getViewState,
        getTrackingStatus,
        registerView,
